...
import { getOrCreateJob, isJobSync } from "./jobs";
...
export async function synchronizeHistory(historyId: number, ownerId: string, supabase: SupabaseClient<Database>) {
    ...
    const galaxyClient = GalaxyClient.getInstance(apiKey, url)
    let historyDb: HistoryWithAnalysisDB = await useDrizzle()
        .select()
        .from(histories)
    ...
    if (historyDb) {
        ...
        const updatedJobs = await synchronizeJobs(historyDb.analyses.id, historyDb.histories.id, ownerId, supabase)
...
export function isHistoryTerminalState(state: HistoryState) {
    return DatasetsTerminalStates.indexOf(state as DatasetTerminalState) !== -1
}
